        ...
        <form action="{{ route('posts.store') }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="mb-4">
                ...
                <textarea name="content" id="content" rows="8" required
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('content') }}</textarea>
                @error('content')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end">
                <a href="{{ route('posts.index') }}" class="mr-4 px-4 py-2 text-gray-600 hover:text-gray-800">Cancelar</a>
